...
def evalSymbReg_with_timeout(individual, variables,line_number, timeout_event, result_queue,wrong_expression):
    try:
...
def evalSymbReg(individual, variables,line_number,wrong_expression):
    ...
    eval_thread = threading.Thread(target=evalSymbReg_with_timeout,
                                   args=(individual, variables,line_number, timeout_event, result_queue,wrong_expression))
    eval_thread.start()

    # Wait for the thread to finish or timeout
    eval_thread.join(timeout=1)  # Timeout set to 3 seconds
...
async def test_program(request: ProgramTestRequest):
    ...
    try:
        ...
        for gen in range(4): 
            # Changer cela au nombre de générations souhaité
